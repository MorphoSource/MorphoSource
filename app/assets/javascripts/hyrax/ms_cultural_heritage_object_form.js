...
$( document ).ready(function() {
  if ($('form[id*="edit_cultural_heritage_object"]').length) { // if CHO form page
    ...
    $('#btn_select_organization').click(function() {
      var org = $('#s2id_cultural_heritage_object_find_organization').select2('data');
      console.log(org);
      ...
      // modify the form
      $('form.edit_cultural_heritage_object input[name^="cultural_heritage_object[organization_id]"]').remove();
      $('<input />').attr('type', 'hidden')
        .attr('name', 'cultural_heritage_object[organization_id][]')
        .attr('value', org.id )
        .appendTo($('form.edit_cultural_heritage_object')
      );   
    });
