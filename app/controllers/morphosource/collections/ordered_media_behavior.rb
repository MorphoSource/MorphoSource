  ...
  module Collections
    module OrderedMediaBehavior
      ...
      def current_page_media_indices
        ...
        page_count = params["per_page"]&.to_i || @collection.media_docs.count
      ...
      def sort_document_list(document_list)
        if @collection.ordered_media.present? && !params["sort"].present?
          ...
          docs_hash = document_list.documents.compact.map { |doc| [doc["id"], doc] }.to_h
      ...
      def full_collection_params(params)
        params.dup.except("per_page").merge( { page: "1", rows: 999999 } )
      end
