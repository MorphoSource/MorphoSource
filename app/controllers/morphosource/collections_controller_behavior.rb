module Morphosource
  module CollectionsControllerBehavior
    ...
    # TODO5: Maybe this should be refactored to use Hyrax 5's member_works approach, despite line 136?
    def query_solr
    ...
    def query_solr_all_results
      response = blacklight_config.repository.search(search_builder.with(params.merge(return_all_fields: true)))
      [response, response.documents]
    end

    def collection_object_ids
      blacklight_config.max_per_page = 999999
      response = blacklight_config.repository.search(media_objects_search_builder_class.new(scope: self, collection: @collection).rows(999999)).response
    ...
    def create_access_facet
      ...
      return if blacklight_config.facet_fields['access'].present?
      blacklight_config.add_facet_field 'access', label: 'Access', query: {
        edit: {
          label: 'Edit',
