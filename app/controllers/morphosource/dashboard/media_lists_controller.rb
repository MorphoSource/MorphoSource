module Morphosource
  module Dashboard
    class MediaListsController < Hyrax::Dashboard::CollectionsController
      ...
      def after_update
        path = main_app.my_media_lists_path
        respond_to do |format|
          format.html { redirect_to path, notice: "#{@collection.title.first}" + t('morphosource.dashboard.media_lists.action.collection_update_success') }
          format.json { render json: @collection, status: :updated, location: path }
        end
      end
