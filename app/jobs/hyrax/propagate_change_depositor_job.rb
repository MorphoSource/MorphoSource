...
module Hyrax
  ...
  # This class intentionally overrides Hyrax's default job so depositor changes
  # still propagate for legacy ActiveFedora works when Valkyrie lookup fails.
  class PropagateChangeDepositorJob < ApplicationJob
    ...
    def perform(work_id, user, reset)
      ...
      call_af(ActiveFedora::Base.find(work_id), user, reset)
