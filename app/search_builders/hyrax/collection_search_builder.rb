module Hyrax
  ...
  class CollectionSearchBuilder < ::SearchBuilder
    ...
    def gated_discovery_filters(permission_types = discovery_permissions, ability = current_ability)
      return super unless permission_types.include?("deposit")
      if permission_types.include?("view")
        filters = ["{!terms f=id}#{collection_ids_for_view.join(',')}"]
      else
        filters = ["{!terms f=id}#{collection_ids_for_deposit.join(',')}"]
      end
      return filters
    end
