module Morphosource
  module OrganizationalAccessBehavior
    ...
    def apply_organization_device_permissions(permission_types = [], ability = current_ability)
      groups = ability.user_groups.dup
      roles = ['_managers','_editors','_downloaders','_viewers']
      groups.select!{|group| roles.any? { |role| group.include?(role) } }
      return [] if groups.empty?

      field = 'media_device_facility_organization_id_ssim'
      collection_ids = groups.map{|group| group.split('_').first}
      ["({!terms f=#{field}}#{collection_ids.join(',')})"]
    end

    def gated_discovery_filters(permission_types = discovery_permissions, ability = current_ability)
      [:apply_user_permissions, :apply_group_permissions, :apply_organization_permissions, :apply_organization_device_permissions].map { |method| send(method, permission_types, ability).reject(&:blank?) }.reject(&:empty?)
    end
  end
end