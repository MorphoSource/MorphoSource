  ...
  module Users
    module EditMedia
      class EditSequentialSectionScansSearchBuilder < Morphosource::Users::EditMediaSearchBuilder
        ...
        self.default_processor_chain += [:apply_modalities_filter, :apply_object_id_filter]
        ...
        def apply_object_id_filter(solr_parameters)
          return unless object_id

          solr_parameters[:fq] ||= []
          solr_parameters[:fq] << object_id_filter
        end

        def object_id_filter
          "(physical_object_id_tesim:(#{object_id}))"
        end

        # nil if collection is empty
        def object_id
          @media_object ||= media_object
          @object_id ||= @media_object.blank? ? nil : @media_object.first["physical_object_id_tesim"].first
        end
