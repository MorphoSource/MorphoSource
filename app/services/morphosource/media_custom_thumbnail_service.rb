...
module Morphosource
  ...
  class MediaCustomThumbnailService
    ...
    # @param custom_thumbnail [ActionDispatch::Http::UploadedFile, Hyrax::UploadedFile] the uploaded thumbnail file
    def initialize(media:, custom_thumbnail: nil)
      @media = media
      if custom_thumbnail.is_a?(Hyrax::UploadedFile)
        @custom_thumbnail = custom_thumbnail.file.file # CarrierWave::SanitizedFile
      else
        @custom_thumbnail = custom_thumbnail # ActionDispatch::Http::UploadedFile
      end
