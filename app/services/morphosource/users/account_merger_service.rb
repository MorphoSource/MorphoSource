module Morphosource
  module Users
    class AccountMergerService
      ...
      def works
        ActiveFedora::Base.where("generic_type_sim:Work AND (depositor_ssim:#{@old_user.ms_id} OR edit_access_person_ssim:#{@old_user.ms_id} OR read_access_person_ssim:#{@old_user.ms_id} OR download_reviewer_ssim:#{@old_user.ms_id} OR proxy_depositor_ssim:#{@old_user.ms_id} OR owner_ssim:#{@old_user.ms_id} OR on_behalf_of_ssim:#{@old_user.ms_id})")
      end
