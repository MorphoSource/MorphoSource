...
#       - Move the inputs to after the hints and errors.
#       - Use 'text-danger' instead of 'text-muted' on all :error components so that
#         inline field validation errors are styled in red rather than plain grey text.
...
SimpleForm.setup do |config|
  ...
  config.wrappers :showcase_default, tag: 'div', class: 'form-group row', error_class: 'has-error' do |b|
    ...
    b.wrapper :label_wrapper, :tag => 'div', :class => 'col-6 showcase-label' do |ba|
      ...
      ba.wrapper :flag_wrapper, :tag => 'span', :class => "badge badge-info" do |bb|
        bb.use :placeholder
      end
    end
