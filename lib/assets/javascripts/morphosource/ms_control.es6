...
export default class RelationshipsControl {
  ...
  displayMembers() {
    if (this.workType == 'organization') {
      this.members.forEach((elem) =>
        this.registry.addResource(new OrganizationResource(elem.id, elem.label, elem.organization_type, elem.institution_code, elem.institution_name, elem.collection_code, elem.description, elem.address, elem.city, elem.state_province, elem.country, elem.contact_person))
      )      
  ...
  addRow() {
    ...
    if (this.workType == 'organization') {
      this.registry.addResource(new OrganizationResource(data.id, data.text, data.organization_type, data.institution_code, data.institution_name, data.collection_code, data.description, data.address, data.city, data.state_province, data.country, data.contact_person));
    } else if (this.workType == 'device') {
