...
$( document ).ready(function() {
  if ($('div[class="submission_flow"]').length) { // check if the page is submission flow page
    ...
    class RawOrDerivedView extends SubmissionView {
      ...
      eventFuncs() {
        ...
        $("select[name='submission[on_behalf_of]']").change(function() {
          ...
          self.triggerChangeVal("select[name='taxonomy_resource[on_behalf_of]']", $(this).val());
          self.triggerChangeVal("select[name='imaging_event_resource[on_behalf_of]']", $(this).val());
          ...
          // Display on_behalf_of value as a read-only data manager field
          ...
          media_owner_wrapper.find('.showcase-value').remove();
        ...
        $("select[name='submission[submission_modality]']").change(function() {
          ...
          self.triggerChangeVal("select[name='imaging_event_resource[ie_modality]']", $(this).val());
    ...
    class ParentMediaView extends SubmissionView {
      ...
      newParentRow(id) {
        var row = '<div class="parent_row" data-id="' + id + '">' +
          '<div class="d-flex align-items-center justify-content-between text-left block mt-1" style="gap: 10px;">' +
          '<span>' + $("input.parent_title").val() + '</span>' +
          '<i id="btn-remove-parent" class="fas fa-trash-alt btn_remove_parent clickable"></i>' +
          '</div></div>';
        return row;
      }
    }
    ...
    class OrganizationView extends SubmissionView {
      ...
      eventFuncs() {
        ...
        $("#submission_organization_search").select2({
          data: objectOrgsData, // Defined in new.html.erb view
          ...
          if (e.choice && e.choice.id) {
            ...
            for (const prop in item) {
              if (item[prop]) {
                ...
                $('#submission_organization_select_display #'+prop).text('--');
              }
            }
    ...
    class TaxonomyView extends SubmissionView {
      ...
      eventFuncs() {
        ...
        $('form#taxonomy_search_form').submit(function(event){
          ...
          if ((data.willCreateTaxonomy) ||
            ...
            if (data.willCreateTaxonomy) {
              data.taxonomyCreateParams = $('#new_taxonomy_resource').serializeArray();
            } else {
      ...
      newTaxonomyRow(id, gbifKey) {
        var row = '<div class="taxonomy_row" data-id="' + id + '" data-gbifkey="' + gbifKey + '">' +
          '<div class="d-flex align-items-center justify-content-between text-left block mt-1" style="gap: 10px;">' +
          '<span>' + $("input.taxonomy_title").val() + '</span>' +
          '<i id="btn-remove-taxonomy" class="fas fa-trash-alt btn_remove_parent clickable"></i>' +
          '</div></div>';
        return row;
      }
    ...
    class ImagingEventView extends SubmissionView {
      ...
      eventFuncs() {
        ...
        $("select[name='imaging_event_resource[ie_modality]']").change(function(event){
          if ($(this).val() !== $('select#submission_submission_modality').val()) {
            ...
            self.triggerChangeVal("select[name='imaging_event_resource[ie_modality]']", $('select#submission_submission_modality').val());
          }
        });

        $('form#new_imaging_event_resource').submit(function(event){
          event.preventDefault();
          ...
          var IE_CreatorsCount = $('[name="imaging_event_resource[creator][]"]').length;
          $('[name="imaging_event_resource[creator][]"]').each(function(index) {
            if ($(this).val() != '') {
          ...
          data.imagingEventCreateParams = $('#new_imaging_event_resource').serializeArray();
          data.savedStep = 8;
