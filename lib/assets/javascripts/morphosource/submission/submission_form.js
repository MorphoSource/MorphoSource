class SubmissionForm {
  ...
  addOrgAsMediaOwner(organization_title, organization_id) {
    var media_owner_wrapper = $('.media_owner_wrapper');
    var on_behalf_of = ($("select[name='submission[on_behalf_of]'] option:selected").val() || $("select[id='batch_submission_on_behalf_of'] option:selected").val());
    if (on_behalf_of != null) {
      // if on_behalf_of is present, add it to the media owner select
      var on_behalf_of_option = `<option value="${on_behalf_of}">${$("select[name='submission[on_behalf_of]'] option:selected").text()}</option>`;
    } else {
      // if on_behalf_of is not present, add the depositor to the media owner select
      var on_behalf_of_option = `<option value="">${$("#media-depositor > div.col-xs-6.showcase-value").text()}</option>`;
    }
    media_owner_wrapper.children().remove();
