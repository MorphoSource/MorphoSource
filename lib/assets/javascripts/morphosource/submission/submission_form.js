     ...
     function(getData){
      ...
      if (getData.default_fields) {
        ...
        if (Object.keys(getData.default_fields).length) {
          ...
          // download_reviewer_mode rides on a hidden input, so it must still be filled below,
          // but on its own it must not raise the header over a section showing nothing.
          var visibleFields = Object.keys(getData.default_fields).filter(function(field) {
            return field !== 'download_reviewer_mode';
          });
