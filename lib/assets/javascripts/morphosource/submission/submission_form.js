class SubmissionForm {
  ...
  setDefaultMediaPermissionFields(selectedReviewerMode = null) {
    let self = this;
    ...
    $.get('/submissions/organization_default_media_fields',
     ...
     function(getData){
      ...
      if (getData.default_fields) {
        ...
        self.configureReviewerMode(getData.default_fields, getData.organization_permissions_mode === 'Require', selectedReviewerMode);
        self.resetDataOwner();
  ...
  configureReviewerMode(defaultFields, mandateValues, selectedMode = null) {
    const mode = document.getElementById('media_download_reviewer_mode');
    if (!mode) return;

    const eligible = defaultFields.download_reviewer_mode === 'object_organization';
    mode.querySelector('option[value="object_organization"]').disabled = !eligible;
    mode.querySelector('option[value="record_users"]').disabled = eligible && mandateValues;
    mode.disabled = false; // The selected mode must be included in the POST.
    mode.value = eligible && (mandateValues || selectedMode !== 'record_users') ? 'object_organization' : 'record_users';
    const picker = document.querySelector('.media_record_download_reviewer_users');
    const updatePicker = function() {
      if (picker) picker.hidden = mode.value === 'object_organization';
    };
    this.canAlertDownloadReviewer = eligible && !mandateValues;
    mode.onchange = () => {
      updatePicker();
      if (mode.value === 'record_users' && this.canAlertDownloadReviewer) {
        this.canAlertDownloadReviewer = false;
        $.alert($('#organization-alert-message').text());
      }
    };
    updatePicker();
    if (picker) {
      picker.querySelectorAll('input, select').forEach(input => {
        input.disabled = eligible && mandateValues;
      });
    }
  }
  ...
  resetFormFromOrg(defaultFields) {
    ...
    this.configureReviewerMode({}, false);
    this.resetDataOwner();
  }
  ...
  resetDataOwner() {
    ...
    var data_owner = ($("select[name='submission[on_behalf_of]'] option:selected").text() || $("select[id='batch_submission_on_behalf_of'] option:selected").text());
    ...
    if (!data_owner) {
      // if on behalf of is empty, the data owner is the data uploader
      data_owner = $("#media-depositor > div.col-6.showcase-value").text();
    }
    media_owner_wrapper.children().remove();
    media_owner_wrapper.append('<div class="col-6 showcase-label">Data Manager</div>');
    media_owner_wrapper.append(`<div class="col-6 showcase-value">${data_owner}</div>`);
   }
  ...
  emptyMediaField(field) {
    ...
    switch(field) {
      case 'download_permission':
        ...
        break;
      case 'record_download_reviewer_users':
  ...
  fillMediaField(field, val, mandateValues) {
    ...
    switch(field) {
      case 'download_permission':
        ...
        const visibilityRadioValue = normalizedVal === 'private' ? 'restricted' : normalizedVal;
        $('form.new_media input[id$="visibility_' + visibilityRadioValue + '"]').trigger('click');
      ...
      case 'record_download_reviewer_users':
        ...
        this.recommendOrRequirePermissions(
          $('form.new_media div.media_record_download_reviewer_users'),
          null,
          mandateValues
        );
        break;
  ...
  recommendOrRequirePermissions(element, val, mandateValues) {
    if (mandateValues) { // required
      if (element.hasClass('media_download_permission')) {
        if (val == 'open') {
          ...
          element.find('div.showcase-value ul#publication-options').before(
            "<div class='permissions-label' style='padding-left: 8px;'><span class='badge badge-default'><i class='fas fa-exclamation-circle fa-lg'></i> Limited to Open or Private</span></div>"
          );
        } else if (val == 'restricted_download') {
          $('form.new_media input[id$="visibility_open"]').prop('disabled', 'disabled');
          element.find('div.showcase-value ul#publication-options').before(
            "<div class='permissions-label' style='padding-left: 8px;'><span class='badge badge-default'><i class='fas fa-exclamation-circle fa-lg'></i> Limited to Restricted or Private</span></div>"
          );
        } else {
        ...
        element.find('div.showcase-value').append(
          "<div class='permissions-label'><span class='badge badge-default'><i class='fas fa-exclamation-circle fa-lg'></i> Mandated by Organization</span></div>"
        );
      }
      ...
      if (element.hasClass('media_record_download_reviewer_users')) {
        this.canAlertDownloadReviewer = true;
        var self = this;
        $('#media_record_download_reviewer_users').one("select2-opening", function() {
      ...
      element.find('div.showcase-value').append(
        "<div class='permissions-label'><span class='badge badge-default'><i class='fas fa-exclamation-circle fa-lg'></i> Recommended by Organization</span></div>"
      );
    }
  }
  ...
  addOrgAsMediaOwner(organization_title, organization_id) {
    var media_owner_wrapper = $('.media_owner_wrapper');
    // If we're on the regular submssion form, get the on_behalf_of value
    if ($("select[name='submission[on_behalf_of]'] option:selected").length > 0) {
      var on_behalf_of_value = $("select[name='submission[on_behalf_of]'] option:selected").val();
      var on_behalf_of_text = $("select[name='submission[on_behalf_of]'] option:selected").text();
    // Otherwise, if we're on the batch submission form, get the batch_submission_on_behalf_of value
    } else if ($("select[id='batch_submission_on_behalf_of'] option:selected").length > 0) {
      var on_behalf_of_value = $("select[id='batch_submission_on_behalf_of'] option:selected").val();
      var on_behalf_of_text = $("select[id='batch_submission_on_behalf_of'] option:selected").text();
    // Otherwise, the data owner is the data depositor
    } else {
      var on_behalf_of_value = '';
      var on_behalf_of_text = $("#media-depositor > div.col-6.showcase-value").text();
    }
    if (on_behalf_of_text.includes('Myself')) {
      on_behalf_of_text = on_behalf_of_text.replace('Myself (', '').replace(')', '');
    }
    var on_behalf_of_option = `<option value="${on_behalf_of_value}">${on_behalf_of_text}</option>`;
    media_owner_wrapper.children().remove();
    media_owner_wrapper.append(`<div class="col-6 showcase-label">
                                  <label class="col-form-label select optional" for="media_owner">Data Manager</label>
