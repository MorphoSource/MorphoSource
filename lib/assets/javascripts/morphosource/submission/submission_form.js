  ...
  setMediaLocalRemoteEvent() {
    $('#tab-local-file-section a').click(function(event) {
      if ($('#media_remote_origin_url').val() != "") {
        $.alert({
          title: '',
          content: 'Please clear the Remote Origin URL if the file is not from a remote location.',
        });
        return false;        
      } else {
        fileOrigin = "local";
    ...
    $('#tab-remote-file-section a').click(function(event) {
      if (justUploaded > 0) {
        $.alert({
          title: '',
          content: 'Please remove the file uploaded from computer or cloud first.',
        });
        return false;
      } else {
        fileOrigin = "remote";
    ...
    $('#media_remote_origin_url').click(function(event) {
// validate url (on change) here ??
    });
  }
