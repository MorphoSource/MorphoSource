...
# Copied from Hyrax 5.0.5 to fix issue with de-duping PG and AF versions of works in find_many_by_ids
module Goddess
  module Query
    ##
    ...
    module MethodMissingMachinations
      ...
      def total_results(result_sets)
        if result_sets.present?
          # Customizing this to make de-dupe of PG and AF versions of works successful

          # Prefer non-wings versions of results
          result_sets.sort_by! do |results|
            results.first.respond_to?(:wings?) && results.first.wings? ? 1 : 0
          end

          result_sets.inject([]) do |out, set|
            unique_in_set = set.reject do |b|
              out.any? { |a| a == b || (a.respond_to?(:id) && b.respond_to?(:id) && a.id.to_s == b.id.to_s) }
            end
            out + unique_in_set
          end
        else
