module Hydra::Works::Characterization
  class FitsDocument
    ...
    PROXIED_TERMS = %i(fits_version format_label file_mime_type exif_tool_version file_size date_modified filename original_checksum date_created rights_basis copyright_basis copyright_note well_formed valid filestatus_message file_title file_author page_count file_language word_count character_count paragraph_count line_count table_count graphics_count byte_order bits_per_sample compression width video_width video_track_width height video_height video_track_height color_space profile_name profile_version orientation color_map image_producer capture_device scanning_software exif_version gps_timestamp latitude longitude character_set markup_basis markup_language audio_duration video_duration bit_depth audio_sample_rate video_sample_rate video_audio_sample_rate channels data_format offset frame_rate audio_bit_rate video_bit_rate track_frame_rate aspect_ratio modality spacing_between_slices secondary_capture_device_manufacturer secondary_capture_device_software_vers file_type_extension image_type study_date series_date content_date study_time series_time content_time accession_number instance_number image_position_patient image_orientation_patient samples_per_pixel photometric_interpretation rows columns pixel_spacing slice_thickness bits_allocated bits_stored high_bit pixel_representation window_center window_width rescale_intercept rescale_slope window_center_and_width_explanation).freeze
    ...
    def photometric_interpretation
      ng_xml.css("fits > metadata > dicom > photometricInterpretation").map(&:text)
    end
