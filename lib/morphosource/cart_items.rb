  ...
  module CartItems
    ...
    def create_cart_item(work)
      ...
      if work.public? || (current_user.can? :download, work.id)
        CartItem.create( { user_id: current_user.ms_id, work_id: work.id, reviewers: Morphosource::DownloadReviewerResolver.new.call(work) } )
      else
        nil
      end
    end
    ...
    def create_cart_item_for_api(work)
      CartItem.create( { user_id: current_user.ms_id, work_id: work.id, reviewers: Morphosource::DownloadReviewerResolver.new.call(work), download_hash: download_hash, download_attempts: 0, in_cart: false, download_method: "API" } )
    end
