  ...
  module Users
    module CartItems
      module ManageRequests
        ...
        def requested_items
          media_ids = media.select{|m| m.reviewer == self.ms_id }.map(&:id)
          items = CartItem.where(work_id: media_ids)
          items.select{ |i| i.date_requested? || i.date_cleared? }
        end

        # media works where user is depositor, owner, or download reviewer
        def media
          @media ||= Morphosource::RequestsSearchService.call({ ms_id: ms_id })
        end
