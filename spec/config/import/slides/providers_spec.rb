...
RSpec.describe 'providers.yml' do
  ...
  describe 'MCZ Special Collections' do
    ...
    it 'has the correct values' do
      ...
      expect(mcz['fileset_accessibility']).to eq(['open'])
      expect(mcz['license']).to eq(['https://creativecommons.org/licenses/by-nc-nd/4.0/'])
      expect(mcz['list_visibility']).to eq('open')
      ...
      expect(mcz['publication_status']).to eq('open')
      ...
      expect(mcz['normalize_permissions']).to eq(false)
    end
  end
end
