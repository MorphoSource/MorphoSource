...
RSpec.describe Morphosource::Admin::DataCurationController, type: :controller do
  ...
  let(:team)                    { Collection.create(title: ['Team'], collection_type_gid: team_collection_type.to_global_id, depositor: admin.ms_id) }
  let!(:project)                { Collection.create(title: ['Project'], collection_type_gid: project_collection_type.to_global_id, depositor: admin.ms_id) }
  ...
  describe 'apply_permission_template' do
    let(:params) { { team_id: team.id, project_id: project.id, old_manager_email: '', email: admin.email,  remove_previous_reviewers: 'true', update_publication_status: 'all' } }
