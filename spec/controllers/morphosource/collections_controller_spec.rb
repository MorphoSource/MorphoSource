...
RSpec.describe Morphosource::CollectionsController, type: :controller do
  ...
  describe 'media_objects_search_builder_class' do
    it { expect(subject.media_objects_search_builder_class).to eq(Morphosource::Collections::MediaObjectsSearchBuilder) }
  end
  ...
  describe 'remove_constraint_url' do
    ...
    context 'collection is an organization' do
      ...
      let(:params)        { { id: organization.id,
                            q: 'term',
                            f: ActionController::Parameters.new( {"human_readable_media_type_ssim"=>["Image"]}),
                            controller: "morphosource/collections/organization_collections" } }

      it 'is the organization remove constraints url' do
        expect(subject.send(:remove_constraint_url, params)).to include("/organizations/#{organization.id}?f%5Bhuman_readable_media_type_ssim%5D%5B%5D=Image&locale=en")
      end
    end
  end
end
