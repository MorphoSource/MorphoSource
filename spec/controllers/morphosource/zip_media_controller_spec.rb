...
RSpec.describe Morphosource::ZipMediaController, type: :controller do
  ...
  describe "GET #zip" do
    ...
    let(:file_set_3)   { FileSet.new(label: 'ms_2.jpg') }
    ...
    describe 'creating or updating cart items' do
      ...
      context 'user is authorized to download the media' do
        ...
        context 'user has downloadable items' do
          ...
          context 'the items have already been downloaded' do
            ...
            it 'creates new downloaded cart items' do
              expect{ process :zip, method: :get, params: { ids: [work1.id, work2.id] } }.to change{CartItem.count}.by(2)
            end
          end
          context 'the items have not been downloaded' do
            it 'marks the items as downloaded' do
              expect{ process :zip, method: :get, params: { ids: [work1.id, work2.id] } }.to change{CartItem.count}.by(0)
        ...
        context 'the user does not have downloadable cart items' do
          before do
            allow(file_set_1).to receive(:parent).and_return(work1)
            allow(file_set_2).to receive(:parent).and_return(work1)
            allow(file_set_3).to receive(:parent).and_return(work2)
          end
          it 'creates new downloaded cart items' do
            expect{ process :zip, method: :get, params: { ids: [work1.id, work2.id] } }.to change{CartItem.count}.by(2)
          end
