FactoryBot.define do
  factory :collection do
    ...
    after(:build) do |collection, evaluator|
      ...
      if evaluator.with_nesting_attributes.present? && collection.collection_type.nestable?
  ...
  factory :user_collection, class: Collection do
    transient do
      user { User.create(email: 'email2@email.com', password: 'password') }
    end
  ...
  factory :typeless_collection, class: Collection do
    ...
    transient do
      user { User.create(email: 'email3@email.com', password: 'password') }
      with_permission_template { false }
      create_access { false }
      do_save { false }
    end
