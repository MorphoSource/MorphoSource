...
RSpec.describe TransferDecisionJob do
  ...
  describe 'perform' do
    ...
    context 'accept' do
      it 'transfers the work and marks the request accepted' do
      ...
      it 'forwards reset' do
        expect_any_instance_of(ProxyDepositRequest).to receive(:transfer!).with(true)
    ...
    context 'request is no longer pending' do
      before { request.update_column(:status, 'accepted') }
    ...
    context 'request no longer exists' do
      it 'is a no-op' do
        expect { described_class.perform_now(request.id + 1, 'accept') }.not_to raise_error
      end
    end
