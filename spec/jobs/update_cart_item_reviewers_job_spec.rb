...
RSpec.describe UpdateCartItemReviewersJob do
  ...
  it 'refreshes orphaned cart items and continues to later items' do
    deleted_user = FactoryBot.create(:contributor)
    orphan = CartItem.create!(user_id: deleted_user.ms_id, work_id: media.id, reviewers: [owner.ms_id])
    later_item = CartItem.create!(user_id: owner.ms_id, work_id: media.id, reviewers: [owner.ms_id])
    deleted_user.delete
    expect(CartItem.find(orphan.id)).not_to be_valid

    described_class.perform_now(media.id)

    expect(orphan.reload.reviewers).to eq([reviewer.ms_id])
    expect(later_item.reload.reviewers).to eq([reviewer.ms_id])
  end

  [ActiveFedora::ObjectNotFoundError, Ldp::Gone].each do |error|
    it "skips Media deleted before execution (#{error})" do
      allow(Media).to receive(:find).with('deleted-media').and_raise(error)
      expect(CartItem).not_to receive(:where)

      expect { described_class.perform_now('deleted-media') }.not_to raise_error
    end

    it "does not suppress #{error} from reviewer resolution" do
      resolver = instance_double(Morphosource::DownloadReviewerResolver)
      allow(Morphosource::DownloadReviewerResolver).to receive(:new).and_return(resolver)
      allow(resolver).to receive(:call).with(media).and_raise(error)

      expect { described_class.perform_now(media.id) }.to raise_error(error)
    end
  end
  ...
  it 'skips writes when the reviewer set is unchanged, including a retry' do
    ...
    item = CartItem.create!(user_id: owner.ms_id, work_id: media.id,
                            reviewers: [other_reviewer.ms_id, reviewer.ms_id], updated_at: 1.day.ago)
    original_updated_at = item.reload.updated_at

    2.times { described_class.perform_now(media.id) }

    expect(item.reload.updated_at).to eq(original_updated_at)
  end
