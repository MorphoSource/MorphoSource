...
RSpec.describe 'morphosource/my/cart/_download_agreements_container.html.erb', type: :view do
  ...
  describe 'Hidden download_agreements container for building the agreement modal' do
      ...
      render 'morphosource/my/cart/download_agreements_container', presenter: media_presenter, item_id: '7'
  	end

		it 'contains proper selectors and content ' do
      expect(page).to have_css('.agreements')
      expect(page).to have_selector('[id="cart-page-download-agreements"][class="download-agreements hide"]')
      expect(page).to have_selector('div[class="agreements"][data-item-id="7"]')
